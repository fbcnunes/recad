...
class ServidorController extends Controller
{
    public function index(Request $request)
    {
        ...
        $ldapPager = $request->session()->get('ldap_pager');

        $servidores = Servidor::query()
            ->with('documentoPessoal')
            ->when($ldapPager, function ($query) use ($ldapPager) {
                $query->where('matricula', $ldapPager);
            })
            ->when($q !== '', function ($query) use ($q) {
                $query->where(function ($sub) use ($q) {
                    $sub->where('nome', 'like', "%{$q}%")
                        ->orWhere('matricula', 'like', "%{$q}%")
                        ->orWhereHas('documentoPessoal', function ($docQuery) use ($q) {
                            $docQuery->where('cpf', 'like', "%{$q}%");
                        });
                });
            })
            ->orderBy('nome')
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            'matricula' => ['required', 'string', 'max:255', 'unique:servidores,matricula'],
        ...
        $servidorData = $this->only($data, [
            'matricula', 'nome', 'pai', 'mae', 'data_nascimento', 'estado_civil', 'conjuge_nome',
        ...
        DB::transaction(function () use (
            ...
            if ($this->hasAny($certidaoData)) {
                CertidaoServidor::create(array_merge($certidaoData, [
                    'servidor_id' => $servidor->id,
                ]));
            }

            if ($this->hasAny($vinculoData)) {
                Vinculo::create(array_merge($vinculoData, [
                    'servidor_id' => $servidor->id,
                ]));
            }

            if ($this->hasAny($contaData)) {
                ContaBancaria::create(array_merge($contaData, [
                    'servidor_id' => $servidor->id,
                ]));
            }
        ...
        return redirect()->route('servidores.index')->with('status', 'Servidor cadastrado com sucesso.');
    }
